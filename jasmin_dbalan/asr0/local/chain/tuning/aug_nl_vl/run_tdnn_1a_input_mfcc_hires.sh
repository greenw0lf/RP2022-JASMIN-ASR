...
train_stage=-10 # from generating egs
...
common_egs_dir=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/software/kaldi/egs/cgn/s5/exp/chain/tdnn1a_sp_bi_epoch3_inputmfcc_hires_only_aug_nl_vl/egs # as we augment nl and vl, egs needs to be prepared from scratch.
...
dir=exp/chain${nnet3_affix}/tdnn${tdnn_affix}_sp_bi_epoch${num_epochs}_input${input_feat_affix}_aug_nl_vl
train_data_dir=data_aug_nl_vl/${train_set}_sp_hires_comb
lores_train_data_dir=data_aug_nl_vl/${train_set}_sp_comb
#train_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires_comb #here we temporarily do not use ivector, as NL and VL ivector extractors are trained separately.
...
if [ $stage -le 18 ]; then
 ...
 if [ $stage -le 21 ]; then
   # decode on VL dev_s and dev_t_16khz, as stage 20 did so on NL
   for x in dev_s dev_t_16khz; do
     vl_data_path=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/software/kaldi/egs/cgn/s5_vl_only/data/
     #nspk=$(wc -l <data/$x/spk2utt)
     nspk=$(wc -l <$vl_data_path/$x/spk2utt)
     [ "$nspk" -gt "$decode_nj" ] && nspk=$decode_nj
     steps/nnet3/decode.sh --num-threads $num_threads_decode --nj $nspk --cmd "$decode_cmd" \
       --acwt 1.0 --post-decode-acwt 10.0 \
       --scoring-opts "--min-lmwt 5 " \
       $dir/graph $vl_data_path/${x}_hires $dir/decode_vl_${x} || exit 1;
     steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" data/lang_s_test_{tgpr,fgconst} \
       $vl_data_path/${x}_hires ${dir}/decode_vl_${x} ${dir}/decode_vl_${x}_rescore || exit
   done
 fi 
