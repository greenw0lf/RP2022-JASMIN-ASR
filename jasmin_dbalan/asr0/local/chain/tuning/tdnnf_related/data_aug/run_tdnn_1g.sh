...
stop_stage=16
nj=30
align_fmllr_lats_stage=0
align_fmllr_lats_nj=60
align_fmllr_lats_max_jobs_run=60
build_tree_max_jobs=10
train_set=train
...
common_egs_dir= #exp/chain/tdnnf_related/aug_related/tdnn1g_bi_epoch3/egs/ #exp/chain/tdnnf_related/tdnn1g_sp_bi_epoch3/egs/
reporting_email=
sp_suffix="_sp" # normally _sp if speed perturbation is applied
comb_suffix="_comb" # normally set to _comb
# LSTM/chain options
train_stage=-10
...
ali_dir=exp/$train_set/${gmm}_ali_${train_set}_aug${sp_suffix}${comb_suffix}
lat_dir=exp/chain${nnet3_affix}/tdnnf_related/aug_related/${gmm}_${train_set}_aug${sp_suffix}${comb_suffix}_lats
dir=exp/chain${nnet3_affix}/tdnnf_related/aug_related/tdnn${affix}${sp_suffix}_bi_epoch${num_epochs}
train_data_dir=data/${train_set}_aug${sp_suffix}_hires${comb_suffix}
train_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_aug${sp_suffix}_${train_set}_aug${sp_suffix}_hires${comb_suffix}
lores_train_data_dir=data/${train_set}_aug${sp_suffix}${comb_suffix}

# note: you don't necessarily have to change the treedir name
# each time you do a new experiment-- only if you change the
# configuration in a way that affects the tree.
tree_dir=exp/chain${nnet3_affix}/tree_aug${sp_suffix}_bi${tree_affix} #same as in chain non-tdnnf models
...
if [ $stage -le 13 ] && [ $stop_stage -gt 13 ]  ; then
  ...
  steps/align_fmllr_lats.sh --stage $align_fmllr_lats_stage --nj $align_fmllr_lats_nj --cmd "$train_cmd --max-jobs-run $align_fmllr_lats_max_jobs_run" ${lores_train_data_dir} \
...
if [ $stage -le 14 ] && [ $stop_stage -gt 14 ]  ; then
  ...
  steps/nnet3/chain/build_tree.sh \
    ...
    --cmd "$train_cmd --max-jobs-run $build_tree_max_jobs" 4000 ${lores_train_data_dir} \
...
if [ $stage -le 18 ] && [ $stop_stage -gt 18 ]  ; then
  ...
  for data in $test_sets; do
      ...
      steps/nnet3/decode.sh \
        ...
        --online-ivector-dir exp/nnet3${nnet3_affix}/ivectors_aug${sp_suffix}_${data}_hires \
