...
if [ $stage -le 14 ]; then
  ...
  if [ -d data/lang_chain ]; then
    if [ data/lang_chain/L.fst -nt data/lang_test/L.fst ]; then
...
if [ $stage -le 19 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 data/lang_test_tgpr $dir $dir/graph
fi
...
if [ $stage -le 20 ]; then
  for x in test; do
    ...
    steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" data/lang_test_{tgpr,fgconst} \
